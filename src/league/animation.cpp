...
void League::Animation::Load(StringView a_FilePath, OnLoadFunction a_OnLoadFunction, void * a_Argument)
{
	struct LoadData
	{
	...
	t_File->Load([](File* a_File, File::LoadState a_LoadState, void* a_Argument)
	{
		...
		std::map<uint32_t, StringView> t_BoneNameHashes;
...
const League::Animation::Bone* League::Animation::GetBone(StringView a_Name) const
{
...
File::LoadState League::Animation::LoadVersion1(const std::map<uint32_t, StringView>& a_BoneNameHashes, File& a_File, size_t & a_Offset)
{
	int32_t t_FileSize;
