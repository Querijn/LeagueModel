...
namespace League 
{
	...
	public:
		...
		const Animation::Bone* GetBone(String a_Name) const;
		float GetDuration() const { return m_Duration; }
	private:
		File::LoadState m_State = File::LoadState::NotLoaded;

		File::LoadState LoadVersion1(const std::map<uint32_t, String>& a_BoneNameHashes, File& a_File, size_t& a_Offset);
